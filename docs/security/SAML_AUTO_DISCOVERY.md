...
default. Java 21.0.10 and later disable `TLS_RSA_*` by default. In the Docker
deployment, the entrypoint generates an appended `java.security.properties`
override before starting the JVM when this profile is selected. It removes
only that wildcard from `jdk.tls.disabledAlgorithms`; all other
disabled-algorithm restrictions remain in force. Non-container launches retain

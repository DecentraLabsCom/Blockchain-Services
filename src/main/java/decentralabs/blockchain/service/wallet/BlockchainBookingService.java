...
public class BlockchainBookingService {
    ...
    public Map<String, Object> getBookingInfo(String wallet, String reservationKey, String labId, String puc) {
        try {           
            ...
            log.error("Error fetching booking info: {}", e.getMessage(), e);
    ...
    private byte[] resolveActiveReservationKey(Diamond diamond, BigInteger labId, String wallet, String puc) {
        try {
            ...
            log.warn("Unable to fetch active reservation key from contract: {}", e.getMessage());
            return null;
        }
    }
    ...
    private Diamond loadDiamondContract(String caller) {
        Web3j web3 = walletService.getWeb3jInstance();
        return Diamond.load(
            contractAddress,
            web3,
            new ReadonlyTransactionManager(web3, caller),

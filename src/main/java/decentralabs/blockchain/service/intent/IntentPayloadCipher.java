...
/** Encrypts the minimal durable execution payload with its own key. */
@Component
public class IntentPayloadCipher {
    ...
    public IntentPayloadCipher(@Value("${intent.payload.encryption-key:}") String configuredKey) {
    ...
    public String encrypt(String plaintext) {
        requireConfigured();
        if (plaintext == null) {
            throw new IllegalArgumentException("Intent payload must not be null");
        }
        try {
            byte[] plaintextBytes = plaintext.getBytes(StandardCharsets.UTF_8);
            if (plaintextBytes.length > MAX_PLAINTEXT_BYTES) {
                throw new IllegalArgumentException("Intent payload is too large");
            }
            ...
            return PREFIX + Base64.getUrlEncoder().withoutPadding().encodeToString(combined.toByteArray());
        } catch (GeneralSecurityException ex) {
            throw new IllegalStateException("Unable to encrypt intent payload", ex);
        }
    }
    ...
    public String decrypt(String ciphertext) {
        ...
        if (ciphertext == null || !ciphertext.startsWith(PREFIX)) {
            throw new IllegalStateException("Unsupported encrypted intent payload format");
        }
        try {
            ...
            if (value.length <= IV_LENGTH || value.length > MAX_ENCRYPTED_VALUE_BYTES) {
                throw new IllegalStateException("Invalid encrypted intent payload");
            }
            ...
            throw new IllegalStateException("Unable to decrypt intent payload", ex);
        }
    }

    private void requireConfigured() {
        if (key == null) {
            throw new IllegalStateException("INTENT_PAYLOAD_ENCRYPTION_KEY must contain exactly 32 bytes");
        }
    }

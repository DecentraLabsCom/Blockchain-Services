...
class SecurityConfigIntegrationTest {
    ...
    @Test
    void labAdminSurface_isAvailableInProviderModeAfterLocalBoundary() throws Exception {
        mockMvc.perform(get("/lab-admin/status")
            ...
            .andExpect(status().isOk())
            .andExpect(content().string("lab-admin-ok"));

        mockMvc.perform(post("/lab-admin/assets")
                .with(anonymous())
                .with(req -> {
                    req.setRemoteAddr("127.0.0.1");
                    return req;
            ...
            .andExpect(status().isOk())
            .andExpect(content().string("lab-admin-asset-ok"));
    ...
    static class TestEndpoints {
        ...
        String labAdminStatus() {
            return "lab-admin-ok";
        }

        @PostMapping("/lab-admin/assets")
        String labAdminAssetUpload() {
            return "lab-admin-asset-ok";
        }

        @DeleteMapping("/lab-admin/assets")
        String labAdminAssetDelete() {
            return "lab-admin-asset-ok";
        }
